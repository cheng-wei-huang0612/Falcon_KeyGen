	...
	const int8_t *restrict f, uint32_t p)
{
	...
	for (size_t u = 0; u < n; u ++) {
		d[u] = mp_set(f[u], p);  // the input f[u] ranges from negative value to positive, here mp_set adjust it to the unsigned representative.
                                 // And store it into tmp array, at pointer d.
	}
}
