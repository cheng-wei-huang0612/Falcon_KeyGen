...
test_Falcon_keygen(void)
{
	...
	int8_t *F = xmalloc(1024);
	int8_t *G = xmalloc(1024);
	void *tmpbytes = xmalloc(24 * 1024 + 18); // We would like to extend this working memory sequence
											  // though using more RAM, it will make the performance better.
											  // We are infact trying to precompute all the twiddle factors 
											  // (possibly some multiplyers during the MODMUL), using more 
											  // memory space to get the best performance.

	// Update the pointer {tmp} for the purpose of alignment.
	uint32_t *tmp = (void *)(((uintptr_t)tmpbytes + 7) & ~(uintptr_t)7);
